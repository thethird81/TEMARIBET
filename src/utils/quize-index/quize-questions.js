export const questions = [
    {
        questionText: 'ቁጥሮችን በቅደም ተከተል ለማስቀመጥ የሚጠቅመን መስመር______ ይባላል።',
        answerOptions: [
            { answerText: 'ሀ) የቁጥር ቤት ', isCorrect: false },
            { answerText: 'ለ) የቁጥር ድምር', isCorrect: false },
            { answerText: 'ሐ) የቁጥር መስመር', isCorrect: true },
            { answerText: 'መ) የቁጥር አሃዝ', isCorrect: false },
        ],
    },
    {
        questionText: 'joystick is a pointing device used to move objects on a screen in any direction',
        answerOptions: [
            { answerText: 'false', isCorrect: false },
            { answerText: 'true', isCorrect: true },

        ],
    },
    {
        questionText: '_____ sometimes called a flight stick is an input device used to control vedio games',
        answerOptions: [
            { answerText: 'A) monitor', isCorrect: false },
            { answerText: 'B) mouse', isCorrect: false },
            { answerText: 'C) keyboard', isCorrect: false },
            { answerText: 'D) joystick', isCorrect: true },
        ],
    },
    {
        questionText: 'ቁጥሮችን ስንደምር ከ ______ አንጀምራለን።',
        answerOptions: [
            { answerText: 'ሀ) ከቀኝ ', isCorrect: true },
            { answerText: 'ለ) ከግራ', isCorrect: false },

        ],
    },



    {
        questionText: '307 + 86',
        answerOptions: [
            { answerText: '386', isCorrect: false },
            { answerText: '394', isCorrect: false },
            { answerText: '373', isCorrect: false },
            { answerText: '393', isCorrect: true },
        ],
    },

    {
        questionText: '4 x 5',
        answerOptions: [
            { answerText: '8', isCorrect: false },
            { answerText: '12', isCorrect: false },
            { answerText: '24', isCorrect: false },
            { answerText: '20', isCorrect: true },
        ],
    },
    {
        questionText: '4 x 6',
        answerOptions: [
            { answerText: '8', isCorrect: false },
            { answerText: '12', isCorrect: false },
            { answerText: '24', isCorrect: true },
            { answerText: '36', isCorrect: false },
        ],
    },
    {
        questionText: '4 x 7',
        answerOptions: [
            { answerText: '8', isCorrect: false },
            { answerText: '12', isCorrect: false },
            { answerText: '24', isCorrect: false },
            { answerText: '28', isCorrect: true },
        ],
    },
    {
        questionText: '4 x 8',
        answerOptions: [
            { answerText: '8', isCorrect: false },
            { answerText: '32', isCorrect: true },
            { answerText: '24', isCorrect: false },
            { answerText: '36', isCorrect: false },
        ],
    },
    {
        questionText: '4 x 9',
        answerOptions: [
            { answerText: '36', isCorrect: true },
            { answerText: '12', isCorrect: false },
            { answerText: '24', isCorrect: false },
            { answerText: '36', isCorrect: false },
        ],
    },
    {
        questionText: '4 x 11',
        answerOptions: [
            { answerText: '30', isCorrect: false },
            { answerText: '36', isCorrect: false },
            { answerText: '44', isCorrect: true },
            { answerText: '48', isCorrect: false },
        ],
    },
    {
        questionText: '4 x 12',
        answerOptions: [
            { answerText: '30', isCorrect: false },
            { answerText: '36', isCorrect: false },
            { answerText: '44', isCorrect: false },
            { answerText: '48', isCorrect: true },
        ],
    },
    {
        questionText: '357 + 299',
        answerOptions: [
            { answerText: '563', isCorrect: false },
            { answerText: '989', isCorrect: false },
            { answerText: '656', isCorrect: true },
            { answerText: '512', isCorrect: false },
        ],
    },

    {
        questionText: '3 x 6',
        answerOptions: [
            { answerText: '24', isCorrect: false },
            { answerText: '15', isCorrect: false },
            { answerText: '27', isCorrect: false },
            { answerText: '18', isCorrect: true },
        ],
    },
    {
        questionText: '3 x 7',
        answerOptions: [
            { answerText: '24', isCorrect: false },
            { answerText: '21', isCorrect: true },
            { answerText: '27', isCorrect: false },
            { answerText: '18', isCorrect: false },
        ],
    },
    {
        questionText: '3 x 8',
        answerOptions: [
            { answerText: '24', isCorrect: true },
            { answerText: '15', isCorrect: false },
            { answerText: '27', isCorrect: false },
            { answerText: '18', isCorrect: false },
        ],
    },
    {
        questionText: '3 x 9',
        answerOptions: [
            { answerText: '24', isCorrect: false },
            { answerText: '15', isCorrect: false },
            { answerText: '27', isCorrect: true },
            { answerText: '18', isCorrect: false },
        ],
    },
    {
        questionText: '4 x 4',
        answerOptions: [
            { answerText: '8', isCorrect: false },
            { answerText: '26', isCorrect: false },
            { answerText: '20', isCorrect: false },
            { answerText: '16', isCorrect: true },
        ],
    },
    {
        questionText: '3 x 11',
        answerOptions: [
            { answerText: '24', isCorrect: false },
            { answerText: '33', isCorrect: true },
            { answerText: '27', isCorrect: false },
            { answerText: '36', isCorrect: false },
        ],
    },
    {
        questionText: '3 x 12',
        answerOptions: [
            { answerText: '36', isCorrect: true },
            { answerText: '30', isCorrect: false },
            { answerText: '27', isCorrect: false },
            { answerText: '33', isCorrect: false },
        ],
    },
    {
        questionText: '2 + 8',
        answerOptions: [
            { answerText: '14', isCorrect: false },
            { answerText: '11', isCorrect: false },
            { answerText: '10', isCorrect: true },
            { answerText: '12', isCorrect: false },
        ],
    },
    {
        questionText: '3 + 8',
        answerOptions: [
            { answerText: '10', isCorrect: false },
            { answerText: '11', isCorrect: true },
            { answerText: '12', isCorrect: false },
            { answerText: '15', isCorrect: false },
        ],
    },
    {
        questionText: '4 + 6',
        answerOptions: [
            { answerText: '10', isCorrect: true },
            { answerText: '12', isCorrect: false },
            { answerText: '13', isCorrect: false },
            { answerText: '14', isCorrect: false },
        ],
    },
    {
        questionText: '5 + 6',
        answerOptions: [
            { answerText: '14', isCorrect: false },
            { answerText: '13', isCorrect: false },
            { answerText: '11', isCorrect: true },
            { answerText: '12', isCorrect: false },
        ],
    },
    {
        questionText: '6 + 6',
        answerOptions: [
            { answerText: '12', isCorrect: true },
            { answerText: '11', isCorrect: false },
            { answerText: '14', isCorrect: false },
            { answerText: '15', isCorrect: false },
        ],
    },
    {
        questionText: '7 + 7',
        answerOptions: [
            { answerText: '11', isCorrect: false },
            { answerText: '14', isCorrect: true },
            { answerText: '13', isCorrect: false },
            { answerText: '12', isCorrect: false },
        ],
    },
    {
        questionText: '7 + 4',
        answerOptions: [
            { answerText: '14', isCorrect: false },
            { answerText: '13', isCorrect: false },
            { answerText: '12', isCorrect: false },
            { answerText: '11', isCorrect: true },
        ],
    },
    {
        questionText: '7 + 5',
        answerOptions: [
            { answerText: '11', isCorrect: false },
            { answerText: '10', isCorrect: false },
            { answerText: '12', isCorrect: true },
            { answerText: '13', isCorrect: false },
        ],
    },
    {
        questionText: '7 + 6',
        answerOptions: [
            { answerText: '11', isCorrect: false },
            { answerText: '13', isCorrect: true },
            { answerText: '12', isCorrect: false },
            { answerText: '14', isCorrect: false },
        ],
    },
    {
        questionText: '7 + 8',
        answerOptions: [
            { answerText: '16', isCorrect: false },
            { answerText: '17', isCorrect: false },
            { answerText: '18', isCorrect: false },
            { answerText: '15', isCorrect: true },
        ],
    },
    {
        questionText: '7 + 9',
        answerOptions: [
            { answerText: '16', isCorrect: false },
            { answerText: '17', isCorrect: false },
            { answerText: '18', isCorrect: true },
            { answerText: '19', isCorrect: false },
        ],
    },
    {
        questionText: '6 + 8',
        answerOptions: [
            { answerText: '12', isCorrect: false },
            { answerText: '13', isCorrect: false },
            { answerText: '14', isCorrect: true },
            { answerText: '15', isCorrect: false },
        ],
    },
    {
        questionText: '6 + 9',
        answerOptions: [
            { answerText: '14', isCorrect: false },
            { answerText: '15', isCorrect: true },
            { answerText: '16', isCorrect: false },
            { answerText: '18', isCorrect: false },
        ],
    },
    {
        questionText: '9 + 2',
        answerOptions: [
            { answerText: '11', isCorrect: true },
            { answerText: '10', isCorrect: false },
            { answerText: '12', isCorrect: false },
            { answerText: '13', isCorrect: false },
        ],
    },
    {
        questionText: '9 + 3',
        answerOptions: [
            { answerText: '10', isCorrect: false },
            { answerText: '11', isCorrect: false },
            { answerText: '12', isCorrect: true },
            { answerText: '13', isCorrect: false },
        ],
    },
    {
        questionText: '9 + 4',
        answerOptions: [
            { answerText: '11', isCorrect: false },
            { answerText: '12', isCorrect: false },
            { answerText: '13', isCorrect: true },
            { answerText: '14', isCorrect: false },
        ],
    },
    {
        questionText: '9 + 5',
        answerOptions: [
            { answerText: '13', isCorrect: false },
            { answerText: '15', isCorrect: false },
            { answerText: '14', isCorrect: true },
            { answerText: '16', isCorrect: false },
        ],
    },
    {
        questionText: '9 + 8',
        answerOptions: [
            { answerText: '16', isCorrect: false },
            { answerText: '17', isCorrect: true },
            { answerText: '18', isCorrect: false },
            { answerText: '19', isCorrect: false },
        ],
    },
    {
        questionText: '9 + 9',
        answerOptions: [
            { answerText: '16', isCorrect: false },
            { answerText: '17', isCorrect: false },
            { answerText: '18', isCorrect: true },
            { answerText: '19', isCorrect: false },
        ],
    },
    {
        questionText: '4 + 5',
        answerOptions: [
            { answerText: '8', isCorrect: true },
            { answerText: '9', isCorrect: false },
            { answerText: '10', isCorrect: false },
            { answerText: '11', isCorrect: false },
        ],
    },
    {
        questionText: '4 + 3',
        answerOptions: [
            { answerText: '7', isCorrect: true },
            { answerText: '6', isCorrect: false },
            { answerText: '5', isCorrect: false },
            { answerText: '8', isCorrect: false },
        ],
    },
    {
        questionText: '5 + 3',
        answerOptions: [
            { answerText: '7', isCorrect: false },
            { answerText: '8', isCorrect: true },
            { answerText: '9', isCorrect: false },
            { answerText: '10', isCorrect: false },
        ],
    },


];
